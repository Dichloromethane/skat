...
#define DERROR_PRINTF(fmt, ...) DEBUG_PRINTF_LABEL(ERROR_C("ERROR"), fmt, ## __VA_ARGS__)
#define DEBUG_PRINTF(fmt, ...) DEBUG_PRINTF_LABEL("DEBUG", fmt, ## __VA_ARGS__)
#define DEBUG_PRINTF_LABEL(label, fmt, ...) DEBUG_PRINTF_RAW(label " (%s): " fmt "\n", __func__, ## __VA_ARGS__)
...
#define ERRNO_CHECK_STR(stmt, str) do { \
		if (stmt)\
