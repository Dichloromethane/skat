...
card_compare_hand(const card_id *const cid1, const card_id *const cid2,
				  const game_rules *const gr, const int pregame) {
  if (!pregame && gr->type == GAME_TYPE_INVALID)
	return 0;

  card c1, c2;
  if (card_get(cid1, &c1) || card_get(cid2, &c2))
	return 0;

  // color order
  if (!pregame && gr->type == GAME_TYPE_NULL) {
	if (c1.cc != c2.cc)
	  return (int) c1.cc - (int) c2.cc;
  } else {// pregame || GAME_TYPE_RAMSCH/GAME_TYPE_GRAND/GAME_TYPE_COLOR

	uint8_t adj_cc1;// adjusted card color 1
	if (c1.ct == CARD_TYPE_B)
	  adj_cc1 = 6;
	else if (!pregame && gr->type == GAME_TYPE_COLOR && c1.cc == gr->trumpf)
	  adj_cc1 = 5;
	else
	  adj_cc1 = c1.cc;

	uint8_t adj_cc2;// adjusted card color 2
	if (c2.ct == CARD_TYPE_B)
	  adj_cc2 = 6;
	else if (!pregame && gr->type == GAME_TYPE_COLOR && c2.cc == gr->trumpf)
	  adj_cc2 = 5;
	else
	  adj_cc2 = c2.cc;
  ...
  // type order
  if (pregame || gr->type == GAME_TYPE_COLOR || gr->type == GAME_TYPE_GRAND
	  || gr->type == GAME_TYPE_RAMSCH)
	return (int) c1.ct - (int) c2.ct;
  else// !pregame && GAME_TYPE_NULL
...
card_compare_stiche(const card_id *const cid1, const card_id *const cid2,
  ...
  return card_compare_hand(cid1, cid2, gr, 0);
}
...
card_compare(const card_id *const cid1, const card_id *const cid2,
  ...
  else if (*mode == CARD_SORT_MODE_PREGAME_HAND)
	return card_compare_hand(cid1, cid2, gr, 1);
  else if (*mode == CARD_SORT_MODE_INGAME_HAND)
	return card_compare_hand(cid1, cid2, gr, 0);
